...
public sealed partial class ReleaseHardeningTests
{
    ...
    public void MainWindowUsesLlamaCppConsoleBrandingAndIcon()
    {
        ...
        Assert.Contains("Title=\"llama.cpp Console v1.0\"", xaml, StringComparison.Ordinal);
        Assert.Contains("AppDisplayName = \"llama.cpp Console\"", source, StringComparison.Ordinal);
        Assert.Contains("AppVersionLabel = \"v1.0\"", source, StringComparison.Ordinal);
    ...
    public void HelpPageGuidesFirstRunWithoutRunningSetupInline()
    {
        ...
        Assert.Contains("Install WSL", source, StringComparison.Ordinal);
        Assert.Contains("Install Ubuntu", source, StringComparison.Ordinal);
        ...
        Assert.Contains("HighlightFirstVisibleHelpButton(_wslInstallButton, _wslCheckUpdatesButton)", source, StringComparison.Ordinal);
        Assert.Contains("HighlightFirstVisibleHelpButton(_wslInstallUbuntuButton, _wslCheckUbuntuUpdatesButton)", source, StringComparison.Ordinal);
    ...
    public void MainWindowKeepsLogDeletionActionsAndReadableRuntimeJobRows()
    {
        ...
        Assert.Contains("Use Log to inspect compiler, git, or WSL output.", source, StringComparison.Ordinal);
    ...
    public void MinimizeBehaviorUsesExplicitTrayAndTaskbarModes()
    {
        ...
        Assert.Equal(42, AppPreferenceService.IntValue("42", fallback: 7));
        Assert.Equal(7, AppPreferenceService.IntValue("bad", fallback: 7));
    ...
    public void OpenCodeLocalProviderCredentialsCanBeRefreshed()
    {
        ...
        var options = config["provider"]?["local-llm-console"]?["options"];

        Assert.True(updated);
        Assert.Equal("http://127.0.0.1:8090/v1", options?["baseURL"]?.ToString());
    ...
    public void PageViewModelsBuildStableRowsFromDomainState()
    {
        ...
        var model = new ModelRecord("model-1", "Qwen Test", Path.Combine(root, "models", "qwen-q4_k_m.gguf"), OwnershipKind.External, "{}", now);
