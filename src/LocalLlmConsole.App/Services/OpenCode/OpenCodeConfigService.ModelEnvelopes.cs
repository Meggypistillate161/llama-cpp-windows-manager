...
public sealed partial class OpenCodeConfigService
{
    ...
    private static JsonObject ModelComparisonEnvelope(JsonObject configOrEnvelope, string providerId, string modelId)
    {
        ...
        return new JsonObject { ["provider"] = new JsonObject { [providerId] = comparisonProvider } };
    }
    ...
    public static string LocalModelIdFor(ModelRecord model)
    {
    ...
    private static string LocalModelId(ModelRecord model)
        => LocalModelIdFor(model);

    private static string LocalProviderIdFor(ModelRecord model, bool useGatewayProvider = false)
        => useGatewayProvider ? LocalProviderId : $"{LocalProviderPrefix}{LocalModelId(model)}";

    private static string LocalProviderNameFor(ModelRecord model, bool useGatewayProvider = false)
        => useGatewayProvider || string.IsNullOrWhiteSpace(model.Name)
            ? LocalProviderName
